...
class DBStorage:
    ...
    def __init__(self):
        ...
        """
        user = os.environ.get('FEEDFLICK_MYSQL_USER')
        password = os.environ.get('FEEDFLICK_MYSQL_PWD')
        host = os.environ.get('FEEDFLICK_MYSQL_HOST', 'localhost')
        db = os.environ.get('FEEDFLICK_MYSQL_DB')
...
# 'mysql+mysqldb://{user}:{password}@{host}/{db}'.format(user, password, host, db),
        ...
        if os.environ.get('FEEDFLICK_ENV') == 'test':
